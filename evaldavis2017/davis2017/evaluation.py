...
class DAVISEvaluation(object):
    ...
    def _evaluate_unsupervised(
        ...
        j_metrics_res = np.zeros(
            (all_res_masks.shape[0], all_gt_masks.shape[0], all_gt_masks.shape[1])
        )
            ...
            for jj in range(all_res_masks.shape[0]):
                if "J" in metric:
                    j_metrics_res[jj, ii, :] = db_eval_iou(
                        all_gt_masks[ii, ...], all_res_masks[jj, ...], all_void_masks
                    )
                if "F" in metric:
        ...
        if "J" in metric and "F" in metric:
            all_metrics = (
                np.mean(j_metrics_res, axis=2) + np.mean(f_metrics_res, axis=2)
            ) / 2
        else:
            all_metrics = (
                np.mean(j_metrics_res, axis=2)
                if "J" in metric
                else np.mean(f_metrics_res, axis=2)
            )
