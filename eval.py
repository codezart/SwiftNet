    ...
    for t in range(1, num_frames):
        ...
        if t - 1 in to_memorize:
            if t - 1 == 0:
                ...
                keys, values = model.module.Memory_update(
...
def save(pred, save, palette):
    for i in range(pred.shape[0]):
        tmp = pred[i]
        img_E = Image.fromarray(tmp.astype(np.uint8))
...
def evaluate(model, Testloader, metric, scale):
    ...
    for V in tqdm.tqdm(Testloader):
        ...
        all_res_masks = np.zeros(
            (num_objects, pred.shape[0], pred.shape[1], pred.shape[2])
        )
    ...
    print("J&F:" + str(g_res[0]), "J:" + str(g_res[1]), "F:" + str(g_res[4]))
    return g_res
