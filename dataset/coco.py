...
class sampled_aug(object):
    def __init__(self):
        self.affinity = iaa.Affine(
            rotate=(-30, 30), shear=(-20, 20), scale={"x": (0.8, 1.2), "y": (0.8, 1.2)}
        )
        self.ela = iaa.ElasticTransformation(alpha=50, sigma=5)

    def __call__(self, image, label):
        image, label = self.affinity(
            image=image, segmentation_maps=label[np.newaxis, :, :, np.newaxis]
        )
...
class Coco_MO_Train(data.Dataset):
    ...
    def Augmentation(self, image, label, sampled_f_m=None):
        ...
        if w < h:
            factor = 480 / w
            image = cv2.resize(
                image, (480, int(factor * h)), interpolation=cv2.INTER_LINEAR
            )
            label = cv2.resize(
                label, (480, int(factor * h)), interpolation=cv2.INTER_NEAREST
            )
        else:
            factor = 480 / h
            image = cv2.resize(
                image, (int(factor * w), 480), interpolation=cv2.INTER_LINEAR
            )
            label = cv2.resize(
                label, (int(factor * w), 480), interpolation=cv2.INTER_NEAREST
            )
        ...
        src_points = np.float32(
            [
                [
                    0 + tx1 + tx1_ * tx1 / (abs(tx1) + 1e-5),
                    0 + ty1 + ty1_ * ty1 / (abs(ty1) + 1e-5),
                ],
                [
                    0 + tx2 + tx2_ * tx2 / (abs(tx2) + 1e-5),
                    h - 1 + ty2 + ty2_ * ty2 / (abs(ty2) + 1e-5),
                ],
                [
                    w - 1 + tx3 + tx3_ * tx3 / (abs(tx3) + 1e-5),
                    h - 1 + ty3 + ty3_ * ty3 / (abs(ty3) + 1e-5),
                ],
                [
                    w - 1 + tx4 + tx4_ * tx4 / (abs(tx4) + 1e-5),
                    0 + ty4 + ty4_ * ty4 / (abs(ty4) + 1e-5),
                ],
            ]
        )
        ...
        if sampled_f_m != None:
            for sf, sm in sampled_f_m:
                ...
                sf_c[sm == 0] = 0
                label[
                    max(0, frame1y - h // 2) : min(384, frame1y + h - h // 2),
                    max(0, frame1x - w // 2) : min(384, frame1x + w - w // 2),
                ][
                    sm[
                        max(0, h // 2 - frame1y) : min(h, h // 2 + 384 - frame1y),
                        max(0, w // 2 - frame1x) : min(w, w // 2 + 384 - frame1x),
                    ]
                    != 0
                ] = np.max(
                    sm
                ...
                sf_c[sm == 0] = 0
                label1[
                    max(0, frame2y - h // 2) : min(384, frame2y + h - h // 2),
                    max(0, frame2x - w // 2) : min(384, frame2x + w - w // 2),
                ][
                    sm[
                        max(0, h // 2 - frame2y) : min(h, h // 2 + 384 - frame2y),
                        max(0, w // 2 - frame2x) : min(w, w // 2 + 384 - frame2x),
                    ]
                    != 0
                ] = np.max(
                    sm
                ...
                sf_c[sm == 0] = 0
                label2[
                    max(0, frame3y - h // 2) : min(384, frame3y + h - h // 2),
                    max(0, frame3x - w // 2) : min(384, frame3x + w - w // 2),
                ][
                    sm[
                        max(0, h // 2 - frame3y) : min(h, h // 2 + 384 - frame3y),
                        max(0, w // 2 - frame3x) : min(w, w // 2 + 384 - frame3x),
                    ]
                    != 0
                ] = np.max(
                    sm
    ...
    def __getitem__(self, index):
                ...
                for sampled_object in sampled_objects:
                    ...
                    try:
                        ...
                        ob_mask = np.lib.pad(
                            ...
                            (
                                (
                                    int((save_h_w - ob_bbox[3]) / 2),
                                    int((save_h_w - ob_bbox[3]) / 2),
                                ),
                                (
                                    int((save_h_w - ob_bbox[2]) / 2),
                                    int((save_h_w - ob_bbox[2]) / 2),
                                ),
                            ),
                            "constant",
        ...
        ob_list = []
        for f in range(3):
            tmp_mask, num_object, ob_list = self.mask_process(
...
if __name__ == "__main__":
    ...
    def get_arguments():
        ...
        parser.add_argument(
            ...
            type=str,
            help="path to coco",
            default="/smart/haochen/cvpr/data/COCO/coco/",
        )
        parser.add_argument(
            "-Ddavis",
            type=str,
            help="path to davis",
            default="/smart/haochen/cvpr/data/DAVIS/",
        )
    ...
    output_dir = args.o
    dataset = Coco_MO_Train(
        "{}train2017".format(coco_root),
        "{}annotations/instances_train2017.json".format(coco_root),
    )
    palette = Image.open(
        "{}Annotations/480p/blackswan/00000.png".format(davis_root)
    ).getpalette()
