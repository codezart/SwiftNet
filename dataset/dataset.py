...
class DAVIS_MO_Test(data.Dataset):
    ...
    def __init__(
        self, root, imset="2017/train.txt", resolution="480p", single_object=False
    ):
        self.root = root
        ...
        with open(os.path.join(_imset_f), "r") as lines:
            for line in lines:
                ...
                _mask = np.array(
                    Image.open(
                        os.path.join(self.mask_dir, _video, "00000.png")
                    ).convert("P")
                )
                ...
                _mask480 = np.array(
                    Image.open(
                        os.path.join(self.mask480_dir, _video, "00000.png")
                    ).convert("P")
                )
    ...
    def load_single_image(self, video, f):
        ...
        N_frames[0] = np.array(Image.open(img_file).convert("RGB")) / 255.0
        try:
