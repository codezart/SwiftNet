...
class RandomSizedCrop(object):
    ...
    def __call__(self, images, labels):
        ...
        for i in range(3):
            ...
            labels[i] = Image.fromarray(labels[i]).resize(
        ...
        for i in range(3):
            ...
            end_h = start_h + 384
            end_w = start_w + 384
            images[i] = images[i][start_h:end_h, start_w:end_w] / 255.0
...
class aug_heavy(object):
    def __init__(self):
        self.affinity = iaa.Sequential(
            [
                ...
                iaa.Sometimes(0.5, iaa.Affine(shear=(-15, 15))),
                iaa.Sometimes(
                    0.5, iaa.Affine(translate_px={"x": (-15, 15), "y": (-15, 15)})
                ),
                iaa.Sometimes(
                    0.5, iaa.Affine(scale={"x": (0.8, 1.2), "y": (0.8, 1.2)})
                ),
            ],
            random_order=True,
        )
    ...
    def __call__(self, images, labels):
        ...
        for i in range(3):
            images[i], labels[i] = self.affinity(
                image=images[i],
