...
def get_arguments():
    ...
    parser.add_argument(
        "-test_iter", type=int, help="evaluat per x iters", default=20000
    )
...
for iter_ in range(args.total_iter):
    ...
    if (iter_ + 1) % save_step == 0 and (iter_ + 1) >= 300000:
        ...
        torch.save(
            model.state_dict(),
            os.path.join(
                args.save, "coco_pretrained_{}_{}.pth".format(args.backbone, iter_)
            ),
        )
